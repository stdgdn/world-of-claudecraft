// Before/after capture for mob_yeti's bespoke Yeti_Attack clip (issue
// #2889 round 2, scripts/build_yeti_anims.mjs). Spawns a frostmane_yeti
// (the MOB_KEYS template that resolves to the mob_yeti visual) next to an
// offline player and lets it close in and swing.
//   BROWSER_PATH=... MODE=before node scripts/yeti_attack_shot.mjs
//   BROWSER_PATH=... MODE=after  node scripts/yeti_attack_shot.mjs
...
await page.evaluate(() => {
  ...
  sim.player.hp = 99999;
  sim.chat('/dev spawn frostmane_yeti');
});
await sleep(500);
...
await page.screenshot({ path: `tmp/yeti-attack-${MODE}.png` });
console.log(`wrote tmp/yeti-attack-${MODE}.png`);
await browser.close();
